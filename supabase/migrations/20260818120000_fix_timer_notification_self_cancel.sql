-- =============================================================================
-- Fix: timer completion write cancels its own due notification (#139)
-- =============================================================================
-- handle_timer_notification_sync's cleanup phase cancelled every pending
-- timer_end row for the user with no scheduled_at guard. When a focus/break
-- interval ends, the client's completion write (completeTimer()) fires this
-- trigger and rebuilds the chain -- including cancelling the row for the
-- interval that just ended, racing the poller for whether the notification
-- ever goes out. Scoping the cleanup to scheduled_at > now() leaves a row
-- that's already due for the poller instead of cancelling it.
-- =============================================================================
...
BEGIN
  ...
  -- Phase 1 (cleanup): cancel every still-pending, not-yet-due timer_end row
  -- for this user. Scoped to scheduled_at > now() (#139) so a row that's
  -- already due is left for the poller instead of being cancelled by the
  -- completion write for the interval that just ended. Phase 2 immediately
  -- rebuilds whatever future chain is still needed, and there is exactly one
  -- user_timer_state row per user (enforced by user_timer_state_user_id_idx),
  -- so this is scoped correctly by construction — no device-local ref
  -- required.
