...
interface GuestData {
  ...
  lastUpdated: string;
  // Ids of habits/tasks generated by getInitialData(), so telemetry can
  // exempt interactions with demo content. Optional: guest sessions
  // persisted before this field existed read back as undefined.
  seed_ids?: string[];
}
...
class MockStore {
  ...
  private getInitialData(): GuestData {
    ...
    return {
      ...
      events,
      lastUpdated: nowIso,
      seed_ids: [...habits.map((h) => h.id), ...tasks.map((t) => t.id)],
    };
  }
  ...
  addTask(
    ...
    // A recurring task's completion spawns the next Occurrence with a fresh
    // id (see taskMutations.toggle). If it continues a seeded demo Series,
    // that Occurrence is still demo content — propagate the exemption so
    // completing it doesn't count as real engagement either.
    if (
  ...
  // True for a habit/task id that came from getInitialData() rather than the
  // guest's own actions — lets telemetry exempt interactions with demo content.
